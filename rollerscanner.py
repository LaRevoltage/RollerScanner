...
if("--help" in sys.argv):
	...
	print("--censys (-c) — scrap data from censys.io, fastet method to gain info")
...
if (response!=0):
	...
	docontinue=input("Do you want to continue considiring that target is marked as DOWN? Y/N: ")
	if(docontinue=="Y" or docontinue=="y"):
		pass
	else:
		print(Fore.RED+"Shutting down")
		exit()
n=int(input("["+Fore.LIGHTCYAN_EX+" ? "+Style.RESET_ALL+"]"+Fore.BLUE+" How many threads you need?(default: 5000): "+Style.RESET_ALL))
...
def portscan(port):
    ...
    try:
        ...
        if choise=="1":
            process=subprocess.Popen(f'''nmap -sV {target} -p {port} -Pn''', shell=True)
...
if(choise=="2"):
	...
	for el in adresses:
		print(Fore.YELLOW+str(adresses.index(el))+Style.RESET_ALL+": "+Fore.GREEN+el+Style.RESET_ALL)
	addrint=int(input(Fore.BLUE+"Censys got those hosts, which would you like to scrap:? "+Style.RESET_ALL))
